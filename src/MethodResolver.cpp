...
Signature Signature::make(MethodCall *mc, Resolver *r) {
    ...
    res.mc = mc;
    if (mc->scope) {
        res.scope = r->resolve(mc->scope.get());
        if (res.scope->type->isPointer()) {
            auto ptr = dynamic_cast<PointerType *>(res.scope->type);
            res.scope = RType(ptr->type);
       }
...
std::vector<Signature> MethodResolver::collect(Signature &sig) {
    ...
    auto mc = sig.mc;
    if (mc->scope) {
        getMethods(sig.scope.value(), mc->name, list, true);
    } else {
...
void MethodResolver::getMethods(RType &rt, std::string &name, std::vector<Signature> &list, bool imports) {
    auto type = rt.type;
    if (type->isPointer()) {
    ...
    if(rt.targetDecl && rt.targetDecl->base){
        auto base=r->resolve(rt.targetDecl->base.get());
