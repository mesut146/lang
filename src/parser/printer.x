...
impl Debug for LambdaType{
  func debug(self, f: Fmt*){
    ...
    if(!self.captured.empty()){
      join(f, &self.captured, ", ");
    }
    f.print(")");
...
impl Debug for Expr{
  func debug(self, f: Fmt*){
    if let Expr::Lit(lit*)=(self){
      ...
      if(print_cst) f.print("Expr::Call{");
      call.debug(f);
    }else if let Expr::MacroCall(call*)=(self){
      if(print_cst) f.print("Expr::MacroCall{");
      call.debug(f);
    }else if let Expr::Par(e*)=(self){
      if(print_cst) f.print("Expr::Par{");
      f.print("(");
      ...
      panic("Expr::debug macro {}", self.line);
    }else if let Expr::Lambda(lm*)=(self){
        if(print_cst) f.print("Expr::Lambda{");
        lm.debug(f);
    }else{
      panic("Expr::debug");
    }
...
impl Debug for Lambda{
  func debug(self, f: Fmt*){
      ...
      if(self.return_type.is_some()){
          f.print(": ");
          f.print(self.return_type.get());
      }
      match (self.body.get()){
          LambdaBody::Expr(ex*)=>f.print(ex),
          LambdaBody::Stmt(st*)=>f.print(st),
      }
  }
}
