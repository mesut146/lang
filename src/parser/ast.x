...
impl ImportStmt{
  ...
  func str(self): String{
    ...
    for(let i = 0;i < self.list.len();++i){
      let part: String* = self.list.get_ptr(i);
      s.append("/");
      s.append(part.str());
    }
    return s;
...
impl Parent{
  ...
  func clone(self): Parent{
    ...
    match self{
        ...
        ,Impl(info*) => return Parent::Impl{info.clone()};
        ,Trait(type*) => return Parent::Trait{type.clone()};
        ,Extern => return Parent::Extern;
    }
  }
}
